...
def recover_json_objects(text):
    """Recover individual JSON objects even if array is cut off."""
    objects = []
    buf = ""
    depth = 0
    in_obj = False
...
def safe_json_load(text):
    """Try full arrays, else fallback to object recovery."""
...
def normalize_items(items, file):
    ...
    for i, it in enumerate(items):
        ...
        if "sentology_uri" in it and "ontology_uri" not in it:
            it["ontology_uri"] = it.pop("sentology_uri")
