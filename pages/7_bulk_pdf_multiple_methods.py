...
st.title("📦 Bulk ABSA — Multi-PDF × Multi-Prompt × Multi-Model Runner")
...
with st.expander("ℹ️ Selected Model Info"):
...
if st.button("🚀 Run Multi-PDF × Multi-Prompt × Multi-Model ABSA", type="primary"):
        ...
        for prompt_name in selected_prompt_names:
            ...
            for model_cfg in selected_models:
                ...
                try:
                    ...
                    fname = (
                        f"absa_{ts}_{pdf.name}_"
                        f"{prompt_name.replace('.md','')}_{model_label}.json"
                    )
    ...
    st.success("✅ Multi-PDF × Multi-Prompt × Multi-Model ABSA Completed")
