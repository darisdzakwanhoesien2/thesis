...
st.subheader("📊 Agreement Distribution by Dimension")

def dist_plot(col, title):
    dist = agree_df[col].value_counts().reindex(["AGREE", "DISAGREE"], fill_value=0)
    st.bar_chart(dist)
...
with c1:
    st.markdown("### 🏷 Aspect")
    dist_plot("aspect_agreement", "Aspect")

with c2:
    st.markdown("### 🧭 ESG")
    dist_plot("esg_agreement", "ESG")

with c3:
    st.markdown("### 😊 Sentiment")
    dist_plot("sentiment_agreement", "Sentiment")

with c4:
    st.markdown("### 🎯 Role")
    dist_plot("role_agreement", "Role")

# =====================================================
# DISAGREEMENT TABLES
# =====================================================

st.subheader("📋 Sentences with Disagreement (Per Dimension)")

def disagreement_table(col, label):
    bad = agree_df[agree_df[col] == "DISAGREE"]
    with st.expander(f"❌ {label} Disagreement — {len(bad)} sentences"):
        if bad.empty:
...
disagreement_table("aspect_agreement", "Aspect")
disagreement_table("esg_agreement", "ESG Category")
disagreement_table("sentiment_agreement", "Sentiment")
disagreement_table("role_agreement", "Role / Tone")

# =====================================================
# FILTERED VIEW
# =====================================================

st.subheader("🔍 Filter by Overall Agreement")
